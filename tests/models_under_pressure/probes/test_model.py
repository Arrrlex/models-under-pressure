...
def test_get_activations(llm_model: LLMModel):
    ...
    activations, token_dict = llm_model.get_batched_activations(dataset, layers=[0])
    ...
    assert "attention_mask" in token_dict
    assert "input_ids" in token_dict
...
def test_get_batched_activations(llm_model: LLMModel):
    ...
    # Mock get_batched_activations to return consistent shapes
    def mock_get_batched_activations(dataset: Mock, layers: list[int]):
        batch_size = len(dataset.inputs)
        mock_activations = torch.ones(
            (1, batch_size, 10, 512)
        )  # (n_layers, batch_size, seq_len, hidden_dim)
        mock_token_dict = {
        ...
        return mock_activations, mock_token_dict

    llm_model.get_batched_activations = mock_get_batched_activations  # type: ignore

    result = llm_model.get_batched_activations(mock_dataset, layers=[0])

    assert isinstance(result[0], torch.Tensor)
    assert isinstance(result[1], dict)
    assert result[0].shape[1] == 3  # Total number of samples
